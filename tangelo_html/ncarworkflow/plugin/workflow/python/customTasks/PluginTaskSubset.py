...
class PluginTaskSubset(pyutilib.workflow.TaskPlugin):
    ...
    def __init__(self,*args,**kwds):
        ...
        self.outputs.declare('subset'+str(self.uid))
    ...
    def execute(self):
        ...
        args = ['/usr/local/ncl/bin/ncl', '-Q', filename, v, swLat, swLon, neLat, neLon, startDate, endDate, wid, tid, "../plugin/workflow/python/customTasks/ncl/subset_time_latlon.ncl"]
        ...
        if nclError or err:
            setattr(self, 'subset'+str(self.uid), error)
        else:
            setattr(self, 'subset'+str(self.uid), result)
