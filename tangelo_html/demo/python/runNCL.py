...
def run(swlat, swlon, nelat, nelon, startyear, endyear):
	...
	return { "alert": "Error subsetting data" }
      ...
      status2 = subprocess.call(["ncl 'infile=\"tmin_subset_time_latlon.nc\"' 'outfile=\"tmin_aggregate_monthly.nc\"' 'varname=\"tmin\"' 'interval=\"month\"' ../ncl/aggregate.ncl"], shell=True)
      if status2 < 0:
	print "Error aggregating data"
	return { "alert": "Error aggregating data" }
      else:
	status3 = subprocess.call(["ncl 'filename=\"tmin_aggregate_monthly.nc\"' ../ncl/narccap_plot_tmin_latlon.ncl"], shell=True)
	if status3 < 0:
